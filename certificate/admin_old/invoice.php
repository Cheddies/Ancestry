...
{
	$fields=array("GRO_orders_id","order_number","billing_address","shipping_address","order_date","GRO_tbl_shipping.description","GRO_tbl_shipping.price","email","phone","order_status","st_ref","st_autho","delivery_method","discount","discount_code");
	...
	switch($cert_ordered['certificate_type'])
	{
		...
		$required_fields=array("birth_reg_year","birth_surname","forenames","DOB","birth_place","fathers_surname","fathers_forenames","mothers_maiden_surname","mothers_surname_birth","mothers_forenames","copies","price_per_copy","scan_and_send");
	...
	CASE DEATH:
		...
		$required_fields=array("registered_year","surname_deceased","forenames_deceased","death_date","relationship_to_deceased","death_age","fathers_surname","fathers_forenames","mothers_surname","mothers_forenames","copies","price_per_copy","scan_and_send");
	...
	CASE MARRIAGE:
		...
		$required_fields=array("registered_year","mans_surname","mans_forenames","womans_surname","womans_forenames","copies","price_per_copy","scan_and_send");
	...
	define("COMPANY_ADDRESS", "Ancestry Shop, PO Box 150, Sandbach, Cheshire, CW11 3WB	                 Phone: 0845 688 5114");
	...
	*/
	...
	//fix for price change on April 1st for UK standard
	
	/*if($order_data['delivery_method']==1 && (strtotime($order_data['order_date']) < strtotime("2010-04-01")) )
	{
		$order_data['price']="19.99";
		
	}*/
	
	//order number is last order before price changed
	
	
	if($order_data['delivery_method']==1 && $order_data['GRO_orders_id']<'105148' )
	{
		$order_data['price']="19.99";
	...
	{
		$order_data['price']=round($order_data['price'] * ( (100 - $order_data['discount']) / 100 ),2); 
	}
	...
	$pdf->CellFitScale(20,5,$currency_prefix . sprintf('%.2f', "$Price_without_VAT"),1,"","C",1);
	$pdf->CellFitScale(20,5,$currency_prefix . sprintf('%.2f', $Price_without_VAT),1,"","C",1);
	$pdf->CellFitScale(23,5,$currency_prefix  . sprintf('%.2f', $VAT),1,"","C",1);
	$pdf->CellFitScale(20,5,$currency_prefix  . sprintf('%.2f', $order_data['price']),1,"","C",1);
	...
	if($cert_details['copies']>1)
	{
		...
		if(isset($cert_details['price_per_copy']) && $cert_details['price_per_copy']>0)
			$cert_cost=$cert_details['price_per_copy'];
		else
			$cert_cost = CERT_COST;
		
		if($order_data['discount']>0)
		{
			$cert_cost=round($cert_cost * ( (100 - $order_data['discount']) / 100 ),2); 
		}
		...
		$pdf->CellFitScale(20,5,$currency_prefix . sprintf('%.2f', $Unit_price),1,"","C",1);
		$pdf->CellFitScale(20,5,$currency_prefix  . sprintf('%.2f', $Total_price),1,"","C",1);
		$pdf->CellFitScale(23,5,$currency_prefix  . sprintf('%.2f', $VAT),1,"","C",1);
		$pdf->CellFitScale(20,5,$currency_prefix  . sprintf('%.2f', $Total_price_2),1,"","C",1);
		
		
	
	}
	
	//scan and send addition
	if($cert_details['scan_and_send']>0)
	{
		$pdf->ln(5);
		$Line_no ++;
		$Item_code ="DIGITALCOPY";
		$Description = "Digital Copy Service";
		$Qty = 1;
		
		//scan and send is not affected by the discount
		/*if($order_data['discount']>0)
		{
			$cert_details['scan_and_send']=round($cert_details['scan_and_send'] * ( (100 - $order_data['discount']) / 100 ),2); 
		}*/
		
		if($show_VAT)
		{
			$Unit_price=$cert_details['scan_and_send']/VAT_RATE;
			$Total_price=$Unit_price * $Qty;		
			$Total_price_2=$Qty*$cert_details['scan_and_send'];
			$VAT=round($Total_price_2-($Total_price_2/VAT_RATE),2);
		}
		else
		{
			$Unit_price=$cert_details['scan_and_send'];
			$Total_price=$Unit_price * $Qty;		
			$Total_price_2=$Qty*$cert_details['scan_and_send'];
			$VAT=0;
		}
	...
	$pdf->CellFitScale(20,5,$currency_prefix . sprintf('%.2f', $Total_price_total),1,"","C",1);
	$pdf->CellFitScale(23,5,$currency_prefix . sprintf('%.2f', $VAT_total),1,"","C",1);
	$pdf->CellFitScale(20,5,$currency_prefix  . sprintf('%.2f', $Total_price_2_total),1,"","C",1);
	...
	*/
	...
	$pdf->CellFitScale(20,5,$currency_prefix  . sprintf('%.2f',  $Total_price_2_total) . $currency_suffix,1,"","C",1);
	...
	$pdf->CellFitScale(20,5,"-" . $currency_prefix  . sprintf('%.2f',  $Total_price_2_total) . $currency_suffix,"BLR","","C",1);
	...
	$pdf->CellFitScale(20,5,$currency_prefix . sprintf('%.2f',  "0"),1,"","C",1);
