<?php
//Added IN clause to WHERE 
//12-02-09 Added getDataWithLimits 
...
class MySQL_DB{
		...
		if($where!='')
		{
			...
			foreach ($where as $clause)
			{
				...
				else
				{
					...
					$clause_parts=explode("LIKE",$clause);
					if(sizeof($clause_parts)==2)
					{
						$clause_parts[0]=mysql_real_escape_string(trim($clause_parts[0]));
						$clause_parts[1]=mysql_real_escape_string(trim($clause_parts[1]));
						$full_clause=$clause_parts[0] . " LIKE " . "'" . $clause_parts[1] . "'";
					}
					$clause_parts=explode("IN",$clause);
					if(sizeof($clause_parts)==2)
					{
						$clause_parts[0]=mysql_real_escape_string(trim($clause_parts[0]));
						$clause_parts[1]=mysql_real_escape_string(trim($clause_parts[1]));
						$full_clause=$clause_parts[0] . " IN " . "(" . $clause_parts[1] . ")";
					}
				}
	...
	function getDataWithLimits($table_name,$fields,$Limit1,$Limit2="",$where="",$group_by="",$join="",$order_by="",$debug=false)
	{
		$table_name=mysql_real_escape_string($table_name);
		$join=mysql_real_escape_string($join);
		$group_by=mysql_real_escape_string($group_by);
	...
	{
	//changed 21-10-08
	//blank dates or dates with 0 values in will return a blank string
	
		if(strlen(trim($date_string))==0)
			return "";
