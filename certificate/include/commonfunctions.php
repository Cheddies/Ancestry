...
function check_entry_date($month,$year)
{
	//return true if the month/year is older than 18th months
	//else return false
	$current_year=date("Y");
	if( ($current_year-$year)>2)
		return true;
		
	$entryTime = mktime(0, 0, 0, $month, "1", $year); 
	$currentTime = time();
	
	if($entryTime < 0) 
		$diffTime = $currentTime + ($entryTime * -1);
	else
		$diffTime = $currentTime - $entryTime;
	
	$year=31536000;//seconds in a year
	
	$month=$year/12;
	
	$diffMonths = $diffTime / $month;
...
function process_form($form_fields,$data,$clean)
{
	...
	foreach($form_fields as $field)
	{
		if(isset($data[$field['name']]))
		{
			...
			elseif($field['reg_ex']!='' && !preg_match($field['reg_ex'],$clean[$field['name']]))
			{
...
function countitems($deptcode) {
	...
	//This function is also duplicated in the checkoutproccess1.php page
		if(preg_match("^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,4})$^", $emailaddress)) {
			return true;
		}
		else {
			return false;
