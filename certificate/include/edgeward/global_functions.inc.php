...
function validationTest($value,$criteria,$match){
	...
	switch (strtolower($criteria)) {
		...
		case 'max':
			...
			break;
		case 'ukpostcode':
			return preg_match('^([Gg][Ii][Rr] 0[Aa]{2})|((([A-Za-z][0-9]{1,2})|(([A-Za-z][A-Ha-hJ-Yj-y][0-9]{1,2})|(([A-Za-z][0-9][A-Za-z])|([A-Za-z][A-Ha-hJ-Yj-y][0-9]?[A-Za-z])))) {0,1}[0-9][A-Za-z]{2})$^',$value);
			break;
		case 'auspostcode':
		...
		case 'uspostcode':
			...
			break;	
		case 'canadapostcode':
			return true; // preg_match('^[ABCEGHJKLMNPRSTVXYabceghjklmnprstvxy]{1}\d{1}[A-Za-z]{1} *\d{1}[A-Za-z]{1}\d{1}$',$value);
			break;	
	}
	return true;
}
