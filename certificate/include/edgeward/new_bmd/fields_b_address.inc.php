...
<?php
echo formInput('text','b_title','b_title','Title',$order['address']['title'],array('attr'=>array('class'=>'add_field short_field','maxlength'=>45),'disabled'=>$disableAddress,'required'=>$titleReq,'error'=>validationError('b_title')));
...
{
		echo formInput('select','b_county','b_county',$countyLabel,$order['billing_address']['county'],array('attr'=>array('class'=>'add_field'),'options'=>$states,'disabled'=>$disableAddress,'required'=>$countyReq,'error'=>validationError('county')));
}
else
{
...
if($order['country'] != '073'){
    echo formInput('select','b_country','b_country','Country',$order['billing_address']['country'],array('attr'=>array('class'=>'add_field'),'options'=>$countries,'disabled'=>$disableAddress,'required'=>!$disableAddress,'error'=>validationError('country')));
}

echo '<div class="input text" id="b_pcode">';
echo formInput('text','b_postcode','b_postcode',$postcodeLabel,$order['billing_address']['postcode'],array('attr'=>array('class'=>'add_field short_field','maxlength'=>10),'div'=>false,'disabled'=>$disableAddress,'required'=>$postCodeReq,'error'=>validationError('b_postcode')));
echo '</div>';
...
if($order['country'] == '073') echo "<input type=\"hidden\" id=\"b_country\" name=\"b_country\" value=\"{$order['country']}\"/>";
