...
//clean POST data
//$post = cleanData($_POST,array('token','formset','formtype','btn','house_no','isAjax','email','tel','forename','surname','address_1','address_2','town','county','postcode','country','billing_different','b_forename','b_surname','b_address_1','b_address_2','b_town','b_postcode','b_country','noemail','norent'));
//IL change
//for use in email clean
$badchars=array("\n","\r","#","$","}","{","^","~","?","*","|","`","&",";","<",",","\\",">","(",")","!","[","]","/","'","\"");

//leave forward slashes
$badcharsAddress=array("\n","\r","#","$","}","{","^","~","?","*","|","`","&","@",";","<",",","\\",">","(",")","!","[","]","'","\"");
...
if(isset($_POST['street_name']))
	$post['street_name'] = form_clean($_POST['street_name'],40);	
if(isset($_POST['house_no']))
	$post['house_no'] = form_clean($_POST['house_no'],40,$badcharsAddress);
if(isset($_POST['address_1']))
	$post['address_1'] = form_clean($_POST['address_1'],40,$badcharsAddress);
if(isset($_POST['address_2']))
	$post['address_2'] = form_clean($_POST['address_2'],40,$badcharsAddress); 
if(isset($_POST['town']))
	$post['town'] = form_clean($_POST['town'],20);
//not required
if(isset($_POST['county']))
	$post['county'] = form_clean($_POST['county'],40);
...
if(isset($_POST['b_street_name']))
	$post['b_street_name'] = form_clean($_POST['b_street_name'],40);	
if(isset($_POST['b_house_no']))
	$post['b_house_no'] = form_clean($_POST['b_house_no'],40,$badcharsAddress);
if(isset($_POST['b_address_1']))
	$post['b_address_1'] = form_clean($_POST['b_address_1'],40,$badcharsAddress);
if(isset($_POST['b_address_2']))
	$post['b_address_2'] = form_clean($_POST['b_address_2'],40,$badcharsAddress);
if(isset($_POST['b_town']))
	$post['b_town'] = form_clean($_POST['b_town'],20);
if(isset($_POST['b_county']))
	$post['county'] = form_clean($_POST['b_county'],40);
...
if($post['formset'] == 'address'){
	...
	$order['order_header']['saddress1'] = $order['add_house_no'] . ' ' . $order['add_street_name'];
	...
	if($order['billing_different'] === '1'){
		...
		$order['order_header']['address1'] = $order['add_b_house_no'] . ' ' . $order['add_b_street_name'];
