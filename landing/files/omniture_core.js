...
Portions Copyright 2004-2011, Ancestry.com*/

// Dummy catch for bad devs who don't define s_account outside of core
s_account = s_account ? s_account : 'ancestryerror';
var s = s ? s : s_gi(s_account,1);

/* CORE OMNITURE FILE*/
/************************** SET DEFAULTS IF NOT SET IN CONFIG FILE **************************/
/* You may add or alter any code config here. */

function s_initial_beacon(){
	s_domain_standards();
	s.t();
}
...
function s_fallback(){
	...
	s.channel = s.channel ? s.channel : 'unknown';
	s.server = s.server ? s.server : window.document.location.hostname;
	s.currencyCode = s.currencyCode ? s.currencyCode : 'USD';
	s.campaign = s.campaign ? s.campaign : s.getQueryParam('o_xid');
	...
	// Set cookie domain period count
	s.cookieDomainPeriods = document.location.hostname.length - document.location.hostname.replace('.', '').length;

	/* Check Page Name and force standard */
	if (typeof s.pageName == "undefined") //||((s.pageName.length - s.pageName.replace(/ : /g,'').length) != 9))
	{
	    s.pageName = 'none : none : none : needs valid page name';
	}

	/* Set Global Props and eVars */

	s.pageName = s.pageName.toLowerCase();

	if ((s.pageName.length - s.pageName.replace(/ : /g, '').length) == 9) {
	    s.hier1 = s.pageName.replace(/ : /g, ':');
	    s.channel = s.pageName.replace(/^.*? : (.*?) : .*$/, '$1');
	    s.prop42 = s.pageName.replace(/^(.*? : .*?) : .*$/, '$1'); 			    //Two Level Hier
	    s.prop43 = s.pageName.replace(/^(.*? : .*?.*? : .*?) : .*$/, '$1'); 	//Three Level Hier  
	    s.eVar42 = s.pageName.replace(/^(.*? : .*?) : .*$/, '$1'); 			    //Two Level Hier
	    s.eVar43 = s.pageName.replace(/^(.*? : .*?.*? : .*?) : .*$/, '$1'); 	//Three Level Hier 
	}

	s.prop41 = s.getPreviousValue(s.pageName, 'gpv_pn'); 					//Previous Page Name
	s.prop44 = dateFormat(now); 										    //Client Timestamp
	s.prop45 = stripParameters(); 									        //PageURL

	s.eVar41 = s.getPreviousValue(s.pageName, 'gpv_pn'); 					//Previous Page Name
	s.eVar44 = dateFormat(now); 											//Client Timestamp
		
	// Move variable definition to non-core & set there
	var track_dls;s.trackDownloadLinks = track_dls ? track_dls : true;
	var track_externals;s.trackExternalLinks = track_externals ? track_externals : true;
	var internal_filters;s.trackInternalFilters = internal_filters ? internal_filters : 'javscript:,*.ancestry.*';
	var leave_query;s.linkLeaveQueryString = leave_query ? leave_query : false; // Check with org to see if this is necessary
}
...
s.p_gpv=new Function("k","u",""+"var s=this,v='',i=u.indexOf('?'),q;if(k&&i>-1){q=u.substring(i+1);v"+"=s.pt(q,'&','p_gvf',k)}return v");s.p_gvf=new Function("t","k",""+"if(t){var s=this,i=t.indexOf('='),p=i<0?t:t.substring(0,i),v=i<0?'T"+"rue':t.substring(i+1);if(p.toLowerCase()==k.toLowerCase())return s."+"epa(v)}return ''");
...
function dateFormat(d) {
	...
	return yyyy + mm + dd + hh + mn; // padding
  };
