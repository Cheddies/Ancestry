...
if( isset($_POST['token']) && isset($_SESSION['token']) && isset($_SESSION['token_time']) && ValidToken($_POST['token'],$_SESSION['token'],$_SESSION['token_time'],0))
{
	...
	//. and - is removed from usual list as it is allowed for email addresses
	...
	if (strlen($fullquerystring) != 9) {
		...
		header('Location:' . "enter_details.php" . $fullquerystring );
		exit();
	}
	
	else {
		//check the tcs box and for prefs
		if(isset($_POST['dprent']))
			$_SESSION['dprent']=form_clean($_POST['dprent'],1);
		else
			unset($_SESSION['dprent']);
			
		if(isset($_POST['dpemail']))
			$_SESSION['dpemail']=form_clean($_POST['dpemail'],1);
		else
			unset($_SESSION['dpemail']);
			
		if(isset($_POST['dpmail']))
			$_SESSION['dpmail']=form_clean($_POST['dpmail'],1);
		else
			unset($_SESSION['dpmail']);
			
		if(isset($_POST['dpwhsmith']))
		{
			$_SESSION['dpwhsmith']=form_clean($_POST['dpwhsmith'],1);
		}
		else
			unset($_SESSION['dpwhsmith']);
			
				
		if(!isset($_POST['tcs']))
		{
			header('location:enter_details.php?error=tcs');
			exit();
		}
		else
		{
		...
		//set the shipping method dependant on country
		
		//$_SESSION['c_shiptype']=PromoShipping($_SESSION['cs_country']);
		$_SESSION['c_shiptype']=14;
		$_SESSION['BillCountryName']=CountryName($_SESSION['c_country']);
		$_SESSION['ShipCountryName']=CountryName($_SESSION['cs_country']);
