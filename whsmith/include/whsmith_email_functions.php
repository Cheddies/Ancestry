...
function  whsmith_email($niceordernumber){
	...
	$query = "SELECT tbl_order_header_whsmith.niceordernum, tbl_order_header_whsmith.order_date, tbl_order_header_whsmith.firstname, tbl_order_header_whsmith.lastname, tbl_order_header_whsmith.address1, tbl_order_header_whsmith.address2, tbl_order_header_whsmith.city, tbl_order_header_whsmith.zipcode, tbl_order_header_whsmith.email, tbl_order_header_whsmith.sfirstname, tbl_order_header_whsmith.slastname, tbl_order_header_whsmith.saddress1, tbl_order_header_whsmith.saddress2, tbl_order_header_whsmith.scity, tbl_order_header_whsmith.szipcode, tbl_shipping.description , tbl_shipping.price, tbl_order_header_whsmith.ordernumber,tbl_order_header_whsmith.state,tbl_order_header_whsmith.sstate
			  FROM tbl_order_header_whsmith
		  	  INNER JOIN tbl_shipping
		      ON tbl_order_header_whsmith.shipvia=tbl_shipping.code
		      WHERE tbl_order_header_whsmith.niceordernum = {$mysql['niceordernumber']};";

	$result = mysql_query($query) or die('Query failed: ' . mysql_error());
	
	$order_data = mysql_fetch_array($result);
	
	$countryquery = "SELECT tbl_countries.country,tbl_order_header_whsmith.niceordernum FROM tbl_countries,tbl_order_header_whsmith WHERE tbl_order_header_whsmith.niceordernum = {$mysql['niceordernumber']} AND tbl_order_header_whsmith.country = tbl_countries.code"; 
	$countryresult = mysql_query($countryquery) or die('Query failed: ' . mysql_error());
	$country=mysql_fetch_row($countryresult);
	$country=$country[0];
	
	$countryquery = "SELECT tbl_countries.country FROM tbl_countries,tbl_order_header_whsmith WHERE tbl_order_header_whsmith.niceordernum = {$mysql['niceordernumber']} AND tbl_order_header_whsmith.scountry = tbl_countries.code"; 
	$countryresult = mysql_query($countryquery) or die('Query failed: ' . mysql_error());
	...
	$basketbody_text = $basketbody_text . pad("GIFTPACK", 20) . "\t" . pad("Ancestry Gift Pack", 45) . "\t" . pad("1", 4) . "\t" . pad("0.00", 6) . "\t" . pad("0" . "%", 8)  ."\t" . pad("0.00", 8) . "\n";
	$basketbody_html = $basketbody_html . "<tr align=\"left\" valign=\"top\" bgcolor=\"#F1EDC5\"><td width=\"50%\">" . "GIFTPACK" . "</td>" . "<td align=\"right\" bgcolor=\"#F1EDC5\">"  . "Ancestry Gift Pack" . "</td>" . "<td align=\"right\" bgcolor=\"#F1EDC5\">" . "1" .  "</td>" . "<td align=\"right\" bgcolor=\"#F1EDC5\">" . "0.00"  ."</td><td align=\"right\" bgcolor=\"#F1EDC5\">" .  "0"  ."%" . "</td><td align=\"right\" bgcolor=\"#F1EDC5\">" . "0.00". "</td></tr>";
