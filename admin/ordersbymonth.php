...
$badchars=array("\n","\r","#","\$","}","{","^","~","?","*","|","`","&",";","<",",","\\",">","(",")","!","[","]","/",".","'","\"");
$clean['PassedOrderYM']=form_clean($_GET['passedOrderYM'],7,$badchars);
	
//Store the passed Year-Month
//$PassedOrderYM=$_GET['passedOrderYM'];

//store year + month seperate for use in mktime
$Year=substr($clean['PassedOrderYM'],0,4);
$Month=substr($clean['PassedOrderYM'],5,2);

if(is_numeric($Year) && is_numeric($Month) && $Month>0 && $Month<13 && $Year>1900 && $Year<9999 )
{
...
$mysql['Year']=mysql_real_escape_string($Year,$link);
$mysql['Month']=mysql_real_escape_string($Month,$link);
...
$query = "SELECT ordernumber, order_date, firstname, lastname,count(DISTINCT(ordernumber)) as NumberofOrders,DAY(order_date) as Day,ROUND(SUM(quantity*price),2) as Total FROM tbl_order_header,tbl_baskets WHERE ordernumber=sessionid AND MONTH(order_date)={$mysql['Month']}  AND YEAR(order_date)={$mysql['Year']} AND authorised=1 GROUP BY day ORDER by Day ASC;";
...
<html>
	...
	<body topmargin="0">
			...
			<div align="center">
				...
				<?php
					...
					for ($count=1;$count<$DaysinMonth+1;$count++)
					{
							...
							echo ("<tr><td width=\"100\" align=\"left\" bgcolor=\"green\" height=\"20\"><h3> <a href=\"ordersbyday.php?passedOrderDate=". $clean['PassedOrderYM'] . "-". $count ."\">".  $count . "</a></h3>". "</td></tr><tr><td bgcolor=\"\">".  "<b>O:</b>:".  $alldaysorders[$count] ."</a>" ."</td></tr><tr><td bgcolor=\"\">".  "<b>T:</b>" ."&pound;". $alldaystotals[$count] . "</td></tr>");
		...
		<tr>	
				<?php 	
				$GraphFile="graph" . basename($MonthTimeStamp)  .".gif";
				$GraphFile2="graph" . basename($MonthTimeStamp)  ."2.gif";
