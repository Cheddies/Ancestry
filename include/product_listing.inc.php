		...
		<div id="product_listing" class="grid_12">
			...
			<ul id="product_list">
				<?php if(!empty($products)):
					...
					$readMore = explode('/',strtolower($product['slug']));
					if($readMore[0] == 'http:'){
						$readMoreLink = $product['slug'];
					} else {
						$readMoreLink = WEBROOT . "products/{$product['deptslug']}/{$product['slug']}";
					}
					?>
					<li class="prod_item grid_12 alpha omega">
						<div class="prodlist_img_wrapper">
							<a href="<?php echo $readMoreLink;?>" title="<?php echo $product['inetsdesc']?>"><img src="<?php echo WEBROOT?>images/products/<?php echo $product['number'];?>_MED.jpg" alt="<?php echo $product['inetsdesc']?>" class="prod_img_s" /></a>
						</div>
						<div class="prodlist_descr_wrapper">
							<h2><a href="<?php echo $readMoreLink;?>" title="<?php echo $product['inetsdesc']?>"><?php echo $product['inetsdesc'];?></a></h2>
							<p class="prod_desc"><?php echo $product['inetshortd'];?> <a href="<?php echo $readMoreLink;?>">Read more.</a></p>
							<?php if($product['buybutton'] !== '0'):?>
								...
								<?php
								$buyBtn = explode('/', strtolower($product['buybutton']));
								if($product['buybutton'] === '1'):?>
									<a href="<?php echo WEBROOT;?>addtobasket.php?code=<?php echo $product['number'];?>" class="buy_now">Buy Now</a>
								<?php elseif($buyBtn[0] == 'http:'):?>
									<a href="<?php echo $product['buybutton'];?>" class="buy_now">Buy Now</a>
								<?php else:?>
