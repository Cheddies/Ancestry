...
function basketItemName($prodCode,$name){
	//Outputs the item name based on selected products with subtitle
	$label = '';
	switch($prodCode){
		case 'FTMDELUXE2009': $label = "<strong>{$name}</strong><br/>Includes 3-month Ancestry.co.uk&trade; Essentials Membership (worth over &pound;32)";
		break;
		case 'FTMDELUXE2010': $label = "<strong>{$name}</strong><br/>Includes 3-month Ancestry.co.uk&trade; Essentials Membership (worth over &pound;32)";
		break;
		case 'FTMPLATINUM2010': $label = "<strong>{$name}</strong><br/>Includes 6-month Ancestry.co.uk&trade; Essentials Membership (worth over &pound;65)";
		break;
		case 'FTMPLAT2010': $label = "<strong>{$name}</strong><br/>Includes 6-month Ancestry.co.uk&trade; Essentials Membership (worth over &pound;65)";
		break;
		case 'FTMWW2010': $label = "<strong>{$name}</strong><br/>Includes 6-month Ancestry.co.uk&trade; World Membership (worth over &pound;113)";
		break;
		case 'GIFTPACK': $label = "<strong>{$name}</strong><br/>Includes 3-month Ancestry.co.uk&trade; Essentials Membership (worth over &pound;32)";
		break;	
		default: $label = $name;
	}
	return $label;
...
$validation = array(
				...
				'house_no'=>array('validation'=>array('required'=>true,'maxlength'=>20),'messages'=>array('maxlength'=>'Maximum 20 characters.')),
				'street_name'=>array('validation'=>array('required'=>true,'maxlength'=>40),'messages'=>array('maxlength'=>'Maximum 40 characters.')),
				...
				'county'=>array('validation'=>array('required'=>false)),
				'country'=>array('validation'=>array('required'=>true)),
				'postcode'=>array('validation'=>array('required'=>true,'maxlength'=>10,'ukpostcode'=>true),'messages'=>array('maxlength'=>'Maximum 10 characters.')),
				...
				'b_house_no'=>array('validation'=>array('required'=>true,'maxlength'=>20),'messages'=>array('maxlength'=>'Maximum 20 characters.'),'depends'=>array('billing_different'=>'1')),
				'b_street_name'=>array('validation'=>array('required'=>true,'maxlength'=>40),'messages'=>array('maxlength'=>'Maximum 40 characters.'),'depends'=>array('billing_different'=>'1')),
...
function basketThumbnail($itemCode,$maxHeight){
	$filename = 'images/products/' . $itemCode . '_TN.jpg';
	$size = getimagesize($filename);
	if($size[1] > $maxHeight){
		$ratio = $size[0]/$size[1];
		$size[0] = round($maxHeight * $ratio);
		$size[1] = $maxHeight;		
	}
	return "<img src=\"$filename\" width=\"{$size[0]}\" height=\"{$size[1]}\"/> ";
}
