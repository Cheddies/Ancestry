...
function CheckForceShipping ($offer_code,$baskets,$shipData,$forceShippingValid) {
	openConn();
	
	//get any restrictions
	$restriction_query = sprintf("SELECT * FROM discount_restriction WHERE discount_code = '%s';",
					mysql_real_escape_string($offer_code));
		
	$notRestricted=true;
		
	$restrictionData = sqlQuery($restriction_query);
					
	if(is_array($restrictionData) && sizeof($restrictionData)>0)
	{
		$notRestricted=false;
		//check there is a valid product in the basket
		foreach ($restrictionData as $restrictionItem)
		{
			foreach($baskets as $item)
			{
				if($item['itemcode']==$restrictionItem['product_code'])
				{
					$notRestricted=true;
					break;
				}
	...
	if($notRestricted)
	{
		$offer_code_sql = sprintf("SELECT force_shipping FROM discount_codes WHERE code = '%s' AND start_date<=CURDATE() AND end_date>=CURDATE()",
					   mysql_real_escape_string($offer_code));
		
		$force_shipping_data = sqlQuery($offer_code_sql);	
	
		if(is_array($force_shipping_data) && $force_shipping_data[0]['force_shipping']!=NULL)
		{
			//ensure the offer code is marked as valid
			$forceShippingValid=true;	
				
			//pull out the forced shipping method
			$force_shipping_code=$force_shipping_data[0]['force_shipping'];
			$force_shipping_sql = sprintf("SELECT s.code AS code, s.price AS price, s.description AS description,s.notes AS notes FROM tbl_shipping AS s WHERE  s.code = '%s';",
						   mysql_real_escape_string($force_shipping_code));
						   							   
			//assign the forced shipping method to the standard shipping data
			$shipData = sqlQuery($force_shipping_sql);
					
			//force the shipping method to be selected
			return $force_shipping_code;
...
function display_banner($banner,$path="banners/")
{
	//03-04-09 
	//Changed code to use urlencode for clickTag parameter
	//to allow for & in URL 
	...
	switch($banner['file_type'])
	{
		case 'I':
			...
			{
				//if theres a link return image wrapped in link tag
				$return="<a href=\"{$banner['link']}\"><img src=\"{$path}image/{$banner['file']}\" alt=\"{$banner['name']}\" /></a>";
			}
			else
			{
				//just return the image
				$return="<img src=\"{$path}image/{$banner['file']}\" alt=\"{$banner['name']}\" />";
			}
		break;
		case 'F':
			$src="{$path}flash/" . $banner['file'];
			if(strlen(trim($banner['link']))>0)
			{
				//if theres a link set the flash file URL to have a clickTag parameter
				
				//urlencode the link to allow for &
				$src=$src."?clickTag=". urlencode($banner['link']);
			}
			$return="<object classid=\"clsid:D27CDB6E-AE6D-11cf-96B8-444553540000\" type=\"application/x-shockwave-flash\" codebase=\"http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=8,0,0,0\" title=\"\" width=\"{$banner['width']}\" height=\"{$banner['height']}\" ><param name=\"movie\" value=\"{$src}\" /><param name=\"quality\" value=\"high\" /><param name=\"wmode\" value=\"opaque\" /><param name=\"allowScriptAccess\" value=\"always\" /><embed allowscriptaccess=\"always\" src=\"{$src}\" wmode=\"opaque\" quality=\"high\" pluginspage=\"http://www.macromedia.com/go/getflashplayer\" type=\"application/x-shockwave-flash\" width=\"{$banner['width']}\" height=\"{$banner['height']}\" /></object>";
...
function banner_choose($banner_data)
{
	//takes in a array of banners with weights
	//and then reurns one of them based on the weighting
	...
	{
		$total_weight=$total_weight+ $banner['weight'];
	}
	
	//choose a random number in the range of the total weighting
	$random=rand(0,$total_weight);
	
	//reset the total weight
	$total_weight=0;
	
	//go through each banner
	//adding up the weights
	//when the random number falls in range 
	//then return the banner at this position
	
	foreach($banner_data as $banner)
	{
		$total_weight=$total_weight+ $banner['weight'];
		if($total_weight>=$random)
			return $banner;
	}
...
function process_form($form_fields,$data,$clean)
{
	...
	foreach($form_fields as $field)
	{
		...
		if(isset($data[$field['name']]))
		{
			...
			if(!isset($field['return']) || $field['return']==true)
		...
		{
			//echo "Data field does not exist<br />";
...
//SEO Functions
function get_page_meta_tags($page,&$keywords,&$description,&$title,$variable="",&$bc_title="")
{
	$link = mysql_connect(DB_HOST,DB_USER, DB_PASS) or die('Could not connect: ' . mysql_error());
	mysql_select_db(DB_NAME) or die('Could not select database');
	...
	if(strlen($variable)>0)
		$query="SELECT keywords,description,title,bc_title FROM meta_tags WHERE page='{$mysql['page']}' AND variable='{$mysql['variable']}'";
	else
		$query="SELECT keywords,description,title,bc_title FROM meta_tags WHERE page='{$mysql['page']}'";
...
function UpdateStock($SessionID,$DB_Name,$DB_User,$DB_Password,$DB_Host)
{
	...
	while($line=mysql_fetch_array($basketresult,MYSQL_ASSOC))
	{
		...
		$update = "UPDATE tbl_products SET units = IF(units<{$mysql['Reduction']},0,units-{$mysql['Reduction']}) WHERE number='{$mysql['Product']}'";
		//echo $update;
...
function countitems($deptcode) {
	...
	function totalpages($numberofproducts, $prodsperpage) {
		$pages = ceil(($numberofproducts)/$prodsperpage);
		return $pages;
	}
...
/**
 * Cleans a string of characters in the $remove array
 * @param string $input
 * @param int $maxlength
 * @param array $remove [optional]
 * @return string
 */
