...
function initPage() {
	...
	$controller = strtolower(str_replace('.php', '', $controller));
	array_shift($urlArray);
	$dept = isset($urlArray[0]) && $urlArray[0] !='' ? strtolower($urlArray[0]) : null;
	$dept = str_replace('.php', '', $dept);
	array_shift($urlArray);
	$prod = isset($urlArray[0]) ? strtolower($urlArray[0]) : null;
	
	//echo "<br/>$controller|$dept|$prod";
	//exit;
	if($controller == 'products'){
		...
		if(!$prod){
			...
			switch($prod){
				case 'surname-book-your-name-in-history':
					$pageTemplate = 'surname_book';
					break;
				case 'regimental-history-books':
					$pageTemplate = 'regimental_book';
					break;
				case 'hall-of-names':
					$pageTemplate = 'hall_of_names';
					break;
                case 'cassini-family-history-maps':
					$pageTemplate = 'cassini';
					break;
			}
			//certificates use another template
			if($dept == 'birth-marriage-death-certificates'){
				$pageTemplate = 'product_cert_detail';
			}
			
			//new product templates
			
			if ($prod == "family-tree-maker-2014-platinum-edition" || $prod == "family-tree-maker-2014-deluxe-edition" || $prod == "family-tree-maker-2012-deluxe-edition" || $prod == "family-tree-maker-2014-world-edition" || $prod == "family-tree-maker-2014-upgrade-edition") {
				$pageTemplate = 'product_new';
			}			
			
			// Use the new template for family tree maker for mac 3
			if ($prod == "family-tree-maker-mac3v") {
				$pageTemplate = 'product_new_mac3v';
			}
...
function getExtraInfo($prodId){
	$link = mysql_connect(DB_HOST, DB_USER, DB_PASS) or die('Could not connect: ' . mysql_error());
	mysql_select_db(DB_NAME) or die('Could not select database');
	$query = "SELECT title, html, html2 FROM tbl_extra_product_info WHERE tbl_extra_product_info.number =  '{$prodId}' LIMIT 1";
	$result = sqlQuery($query);
	$rows=count($result);
	if(!$rows) return array();
	// Closing connection
