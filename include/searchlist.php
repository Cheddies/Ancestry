...
<?php
	...
	//22-12-08 - Edit for multiple products appearing when in 2 different departments
	//added DISCTINT to number
	$SELECT = "SELECT DISTINCT tbl_products.number,inetfdesc,relateditems,units,tbl_products.expectdate,tbl_products.units, tbl_products.inetsdesc, tbl_products.price, tbl_products.compareprice, tbl_products.inetshortd, tbl_products.inetthumb, tbl_products.mastersub,presell FROM tbl_products INNER JOIN tbl_products_departments ON tbl_products.number=tbl_products_departments.number  WHERE (tbl_products.units > 0 OR lower(tbl_products.presell) = 'pre'   OR tbl_products.mastersub='M')  AND tbl_products.mastersub <> 'S' AND tbl_products.inetsell = 1 "  ;
