...
<?php
	...
	{
		if (StockCheckBasket(session_id(),DB_NAME,DB_USER,DB_PASS,DB_HOST)==false)
		{
...
?>	
<form action="checkoutprocess1.php" method="post">
...
<p>	
	...
	<label for="fname">*First Name:</label>
		...
		?>
			...
			<?php
			$query1 = "SELECT country,code,visible FROM tbl_countries WHERE visible='1' ORDER BY country ASC;";
		...
		<span id="ship_tick_box"><input type="checkbox" name="same" value="on" <?php if ($_SESSION['c_same'] == "on") { Escaped_Echo ( "checked"); } ?> />Shipping address is same as the billing address</span>
	...
	<div class="checkout_box half">	
		...
		?>
		<br />
		<input type="text" size="15" maxlength="15" id="sfname" name="sfname" value="<?php Escaped_Echo($_SESSION['cs_name']) ?>" />
		<br />
		<label for="ssurname">*Surname:</label>
		<?php 
		if (isset($_GET['shipsurname']) == true) 
		{ 
		?>
			<span class="error">
				<?php Escaped_Echo("Required Field");?>
		...
		?>
		<br />
		<input type="text" size="15" maxlength="20" id="ssurname" name="ssurname" value="<?php Escaped_Echo($_SESSION['cs_sname']) ?>" />
		<br />		
		<label for="sadd1">*Address 1:</label>
		<?php 
		if (isset($_GET['shipadd1']) == true)
		{ 
		?>
			<span class="error">
				<?php Escaped_Echo("Required Field");?>
		...
		?>
		<br />
		<input type="text" size="40" maxlength="40" id="sadd1" name="sadd1" value="<?php Escaped_Echo($_SESSION['cs_add1']) ?>" />
		<br />
		<label for="sadd2">Address 2:</label>
		<br />
		<input type="text" size="40" maxlength="40" id="sadd2" name="sadd2" value="<?php Escaped_Echo($_SESSION['cs_add2']) ?>" />
		<br />
		<label for="scity">*City:</label>
		<?php 
		if (isset($_GET['shipcity']) == true)
		{ 
		?>
			<span class="error">
				<?php Escaped_Echo("Required Field");?>
		...
		?>
		<br />
		<input type="text" size="20" maxlength="20" id="scity" name="scity" value="<?php Escaped_Echo($_SESSION['cs_city']) ?>" />
		
		<label style="display:none" for="sstate">State(required for US + Canada):</label><?php if (isset($_GET['shipstate']) == true) { echo "<span class=\"error\">Select a state</span>";} ?>
		
		<select style="display:none" id="sstate" name="sstate">
		<?php
		...
		<option value="">Select a State</option>
		<?php
			while ($line = mysql_fetch_array($resultstate2, MYSQL_ASSOC)) {
				if($line['code']==$_SESSION['cs_state'])
				{
		?>
			<option selected value="<?php echo $line['code'];?>"> <?php echo $line['name']?> (<?php echo $line['code']?>)</option>
		<?php
		...
		<?php
				...
				mysql_free_result($resultstate2);
		?>
		</select>
		<br />
		<label for="spostcode">*Postcode/Zipcode:</label>
		<br />
		<input type="text" size="10" maxlength="10" id="spostcode" name="spostcode" value="<?php Escaped_Echo($_SESSION['cs_postcode']) ?>" />
		<?php
		if (isset($_GET['shippostcode']) == true)
		{
		?>
			<span class="error">
				<?php Escaped_Echo("Required Field");?>
		...
		if ( (isset($_SESSION['cs_country'])) && ($_SESSION['cs_country']!="") )
		{
		?>
			<option value="">Select a Country</option>
			<?php
			$query1 = "SELECT country,code,visible FROM tbl_countries WHERE visible='1' ORDER BY country ASC;";
			$countryresult2= mysql_query($query1) or die('Query failed:<br /> ' . mysql_error());
			while ($line = mysql_fetch_array($countryresult2, MYSQL_ASSOC)) {
				if($line['code']==$_SESSION['cs_country'])
				{
			?>
					<option selected value="<?php Escaped_Echo($line['code']);?>"> <?php Escaped_Echo($line['country'])?></option>
			<?php
			...
			//mysql_free_result($countryresult2);
			?>
		<?php
		}
		else
		...
		?>	
			...
			<?php 
			if (isset($_GET['shipcountry']) == true) 
			{
			?>
				<span class="error">
					<?php Escaped_Echo("Select a country");?>
			...
			?>
				<?php
		if(isset($_GET['shipping_restrict']))
		{
		?>
		<span class="error">
			<?php escaped_echo("Your choosen products can not be shipped to this country")?>
		</span>
		<?php
		}
		?>
